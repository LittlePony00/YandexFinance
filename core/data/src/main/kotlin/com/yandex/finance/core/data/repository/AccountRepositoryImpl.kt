...
class AccountRepositoryImpl(
    ...
    override suspend fun fetchAccounts(): Result<List<AccountDetailed>> {
        return accountService.fetchAccounts().asExternalResult { accountList ->
            accountList.map { it.asExternalModel() }
        }
    }

    override suspend fun createAccount(body: AccountWithoutId): Result<AccountDetailed> {
        return accountService.createAccount(body.asNetworkModel()).asExternalResult { 
            it.asExternalModel() 
        }
    }

    override suspend fun fetchAccount(id: String): Result<MainAccount> {
        return accountService.fetchAccount(id).asExternalResult { 
            it.asExternalModel() 
        }
    }
    ...
    override suspend fun updateAccount(
        ...
        return accountService.updateAccount(id, body.asNetworkModel()).asExternalResult { 
            it.asExternalModel() 
        }
    }
    ...
    override suspend fun fetchAccountHistory(id: String): Result<AccountHistory> {
        return accountService.fetchAccountHistory(id).asExternalResult { 
            it.asExternalModel() 
        }
    }
}
